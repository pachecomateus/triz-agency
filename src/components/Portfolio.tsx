...
export default function Portfolio() {
  ...
  return (
    <section className="relative bg-gray">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 h-[1300px] lg:h-[1130px]">
        {mosaic.map((item, index) => (
          <Link key={item.title + index} href={item.href} target="_blank" title={item.title} className="relative group w-full h-full">
            ...
            <Image src={item.image} alt={item.title} fill quality={100} className="object-cover object-center" />
          </Link>
        ))}
      </div>
    </section>
